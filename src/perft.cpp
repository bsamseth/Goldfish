...
void Perft::run() {
    std::unique_ptr<Position> position(new Position(Notation::to_position(Notation::STANDARDPOSITION)));
    int depth = MAX_DEPTH;

    std::cout << "Testing " << Notation::from_position(*position) << " at depth " << depth << std::endl;

...
template<> constexpr bools is_valid_on_empty_board<PieceType::PAWN>(Move m) {
...
template<> constexpr bools is_valid_on_empty_board<PieceType::KNIGHT>(Move m) {
    const from = from_sq(m), to = to_sq(m);
    const file_diff = std::abs(Squares::file_of(from) - Squares::file_of(to));
    const rank_diff = std::abs(Squares::rank_of(from) - Squares::rank_of(to));
    return (rank_diff == 2 && file_diff == 1) || (rank_diff == 1 && file_diff == 2);
}
template<> constexpr bools is_valid_on_empty_board<PieceType::BISHOP>(Move m) {
    const from = from_sq(m), to = to_sq(m);
    const file_diff = std::abs(Squares::file_of(from) - Squares::file_of(to));
    const rank_diff = std::abs(Squares::rank_of(from) - Squares::rank_of(to));
    return rank_diff == file_diff;
}
template<> constexpr bools is_valid_on_empty_board<PieceType::ROOK>(Move m) {
    const from = from_sq(m), to = to_sq(m);
    const file_diff = std::abs(Squares::file_of(from) - Squares::file_of(to));
    const rank_diff = std::abs(Squares::rank_of(from) - Squares::rank_of(to));
    return rank_diff == 0 || file_diff == 0;
}
template<> constexpr bools is_valid_on_empty_board<PieceType::QUEEN>(Move m) {
    return is_valid_on_empty_board<PieceType::BISHOP>(m) || is_valid_on_empty_board<PieceType::ROOK>(m);
}
template<> constexpr bools is_valid_on_empty_board<PieceType::KING>(Move m) {
    const from = from_sq(m), to = to_sq(m);
    const file_diff = std::abs(Squares::file_of(from) - Squares::file_of(to));
    const rank_diff = std::abs(Squares::rank_of(from) - Squares::rank_of(to));

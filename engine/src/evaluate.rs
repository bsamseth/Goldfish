/// Evaluate a `Board`.
///
/// The evaluaiton is based on [`PeSTO's Evaluation Function`](cp-wiki).
...
///
/// [cp-wiki]: https://www.chessprogramming.org/PeSTO%27s_Evaluation_Function
...
type Psqt = [[[i32; 64]; 6]; 2];
const fn init_table() -> (Psqt, Psqt) {
    let mut mg_psqt = [[[0; 64]; 6]; 2];
    let mut eg_psqt = [[[0; 64]; 6]; 2];
...
const PSQT: (Psqt, Psqt) = init_table();
...
pub const fn piece_value(piece: Piece) -> Value {
    let value = match piece {
        Piece::Pawn => EG_VALUE[0],
        Piece::Knight => EG_VALUE[1],
        Piece::Bishop => EG_VALUE[2],
        Piece::Rook => EG_VALUE[3],
        Piece::Queen => EG_VALUE[4],
        Piece::King => 20000,
    };
...
fn evaluate_piece_square_tables(board: &Board) -> Value {
    ...
    for s in (0..64).map(|s| unsafe {
        ...
        mg[color as usize] += PSQT.0[color as usize][piece as usize][s.to_index()];
        eg[color as usize] += PSQT.1[color as usize][piece as usize][s.to_index()];
        gamephase += match piece {
    ...
    let side_to_move = usize::from(board.side_to_move() != Color::White);
    let mg_score = mg[side_to_move] - mg[side_to_move ^ 1];
    let eg_score = eg[side_to_move] - eg[side_to_move ^ 1];
    let mg_phase = gamephase.min(24);
    let eg_phase = 24 - mg_phase;
    let final_score = (mg_score * mg_phase + eg_score * eg_phase) / 24;

    Value::new(
        final_score
            .min(i32::from(i16::MAX))
...
mod tests {
    ...
    #[test]
    fn test_eval_even_position() {
        let board = Board::default();
        let score = board.evaluate();
        assert_eq!(score, Value::ONE);
    }

    #[test]
    fn test_eval_after_e4() {
        let mut board = Board::default();
        board = board.make_move_new("e2e4".parse().unwrap());
        let score = board.evaluate();
        assert_eq!(score, Value::new(-31));
    }
}

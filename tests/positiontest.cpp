...
TEST(positiontest, test_equals) {
    ...
    // FEN test
    Position position3(Notation::to_position("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1"));
    EXPECT_EQ(position1, position3);

    Position position4(Notation::to_position("rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR w KQkq - 0 1"));
    Position position5(Notation::to_position("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR b KQkq - 0 1"));
    Position position6(Notation::to_position("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQ - 0 1"));
    Position position7(Notation::to_position("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 1 1"));
    Position position8(Notation::to_position("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 2"));
...
TEST(positiontest, test_active_color) {
    ...
    // Move white pawn
    int move = Moves::value_of(MoveType::NORMAL, Square::A2, Square::A3, Piece::WHITE_PAWN, Piece::NO_PIECE,
                              PieceType::NO_PIECE_TYPE);
    position.make_move(move);
    EXPECT_EQ(Color::BLACK, position.active_color);

    // Move black pawn
    move = Moves::value_of(MoveType::NORMAL, Square::B7, Square::B6, Piece::BLACK_PAWN, Piece::NO_PIECE,
                          PieceType::NO_PIECE_TYPE);
    position.make_move(move);
...
TEST(positiontest, test_half_moveClock) {
    ...
    // Move white knight
    move = Moves::value_of(MoveType::NORMAL, Square::B1, Square::C3, Piece::WHITE_KNIGHT, Piece::NO_PIECE,
                          PieceType::NO_PIECE_TYPE);
    position.make_move(move);
...
TEST(positiontest, test_is_repetition) {
    ...
    // Move white knight
    int move = Moves::value_of(MoveType::NORMAL, Square::B1, Square::C3, Piece::WHITE_KNIGHT, Piece::NO_PIECE,
                              PieceType::NO_PIECE_TYPE);
    position.make_move(move);

    // Move black knight
    move = Moves::value_of(MoveType::NORMAL, Square::B8, Square::C6, Piece::BLACK_KNIGHT, Piece::NO_PIECE,
                          PieceType::NO_PIECE_TYPE);
    position.make_move(move);

    // Move white knight
    move = Moves::value_of(MoveType::NORMAL, Square::G1, Square::F3, Piece::WHITE_KNIGHT, Piece::NO_PIECE,
                          PieceType::NO_PIECE_TYPE);
    position.make_move(move);

    // Move black knight
    move = Moves::value_of(MoveType::NORMAL, Square::C6, Square::B8, Piece::BLACK_KNIGHT, Piece::NO_PIECE,
                          PieceType::NO_PIECE_TYPE);
    position.make_move(move);

    // Move white knight
    move = Moves::value_of(MoveType::NORMAL, Square::F3, Square::G1, Piece::WHITE_KNIGHT, Piece::NO_PIECE,
                          PieceType::NO_PIECE_TYPE);
    position.make_move(move);
...
TEST(positiontest, test_pawn_doubleMove) {
    ...
    int move = Moves::value_of(MoveType::PAWN_DOUBLE, Square::A2, Square::A4, Piece::WHITE_PAWN, Piece::NO_PIECE,
                              PieceType::NO_PIECE_TYPE);
    position.make_move(move);
...
TEST(positiontest, test_pawn_promotionMove) {
    ...
    int move = Moves::value_of(MoveType::PAWN_PROMOTION, Square::A7, Square::A8, Piece::WHITE_PAWN, Piece::NO_PIECE,
                              PieceType::QUEEN);
    position.make_move(move);

    EXPECT_EQ(Piece::WHITE_QUEEN, position.board[static_cast<int>(Square::A8)]);
...
TEST(positiontest, test_enpassantMove) {
    ...
    int move = Moves::value_of(MoveType::EN_PASSANT, Square::E4, Square::D3, Piece::BLACK_PAWN, Piece::WHITE_PAWN,
                              PieceType::NO_PIECE_TYPE);
    position.make_move(move);

    EXPECT_EQ(Piece::NO_PIECE, position.board[static_cast<int>(Square::D4)]);
    EXPECT_EQ(Piece::BLACK_PAWN, position.board[static_cast<int>(Square::D3)]);
...
TEST(positiontest, test_castling_move) {
    ...
    int move = Moves::value_of(MoveType::CASTLING, Square::E1, Square::C1, Piece::WHITE_KING, Piece::NO_PIECE,
                              PieceType::NO_PIECE_TYPE);
    position.make_move(move);

    EXPECT_EQ(Castling::NO_CASTLING, position.castling_rights &Castling::WHITE_QUEEN_SIDE);

    position.undo_move(move);

    EXPECT_EQ("r3k2r/8/8/8/8/8/8/R3K2R w KQkq - 0 1", Notation::from_position(position));
    EXPECT_EQ(zobrist_key, position.zobrist_key);

    position = Notation::to_position("r3k2r/8/8/8/8/8/8/R3K2R w KQkq - 0 1");
    zobrist_key = position.zobrist_key;

    move = Moves::value_of(MoveType::CASTLING, Square::E1, Square::G1, Piece::WHITE_KING, Piece::NO_PIECE,
                          PieceType::NO_PIECE_TYPE);
    position.make_move(move);

    EXPECT_EQ(Castling::NO_CASTLING, position.castling_rights &Castling::WHITE_KING_SIDE);

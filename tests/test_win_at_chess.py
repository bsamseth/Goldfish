...
def solve(engine: chess.engine.SimpleEngine, epd: str, max_time: int = 5):
    board = chess.Board()
    case = board.set_epd(epd)
    bms = case["bm"]
    assert isinstance(bms, chess.Move) or (
        isinstance(bms, list) and all(isinstance(mv, chess.Move) for mv in bms)
    ), f"bm must be a move or a list of moves, not {bms!r}"
    if isinstance(bms, chess.Move):
        bms = [bms]

    with engine.analysis(
        board,
        info=chess.engine.INFO_BASIC | chess.engine.INFO_PV,
        ...
        for info in analysis:
            ...
            if best_move is not None and best_move[0] in bms:
                return True

    # TODO: Add a sucesss case here when the case has a `dm` (direct mate) tag and the engine finds it
    # using a `go mate` command.
    return False
...
1nbq1r1k/3rbp1p/p1p1pp1Q/1p6/P1pPN3/5NP1/1P2PPBP/R4RK1 w - - bm Nfg5; dm 4; id "WAC.293";
...
b2b1r1k/3R1ppp/4qP2/4p1PQ/4P3/5B2/4N1K1/8 w - - bm g6; id "WAC.300";""".strip().split(
    "\n"
)


parser = argparse.ArgumentParser(
    description='Run the "Win at Chess" test suite against a given engine.'
)
parser.add_argument(
    "engine",
    type=str,
    help="The executable to test. It should be a UCI chess engine.",
)
...
for i, epd in enumerate(WIN_AT_CHESS_NEW, start=1):
    ...
    print(
        f"{i:3d} GRADE {epd}".replace(
            "GRADE", (["\033[0;31mFAIL", "\033[0;32mPASS"][int(passed)] + "\033[0m")
        )
    )
print("\033[0;31mFailed positions:\033[0m\n", "\n".join(failed))
print(
    "\033[0;33mTotal Score:\033[0m "
    + f"{total}/{len(WIN_AT_CHESS_NEW)} = {round(total / len(WIN_AT_CHESS_NEW) * 100, 2)} %"
)
engine.quit()
